...
class Encoder(torch.nn.Module):
    def __init__(self, data_features: dict, h_params: dict):
        ...
        self.power_embedding = nn.Embedding(self.data_features["n_powers"], self.h_params['power_emb_size'])
        uniform_(self.power_embedding.weight, -1, 1)
        
        self.season_embedding = nn.Embedding(self.data_features["n_seasons"], self.h_params['season_emb_size'])
        uniform_(self.season_embedding.weight, -1, 1)
        
        self.film_output_dims = (
            [self.h_params['gcn_size']] * (self.h_params['n_graph_conv'] - 1) + 
        ...
        self.board_encoding_film_weight_layer = nn.Linear(
            in_features = self.h_params['power_emb_size'] + self.h_params['season_emb_size'],
        ...
        self.order_embedding = torch.empty((ORDER_VOCABULARY_SIZE, self.h_params['order_emb_size'])).uniform_(-1, 1)
        
        # Creating candidate embedding
        self.candidate_embedding = torch.empty((ORDER_VOCABULARY_SIZE, self.h_params['lstm_size'] + 1)).uniform_(-1, 1)
    ...
    def _postprocess_input(self, inputs: dict):
        ...
        board_alignments = torch.reshape(
            board_alignments, (self.h_params["batch_size"], -1, self.data_features["n_nodes"])
        )
              
        board_alignments /= torch.maximum(
            torch.tensor(1.), torch.sum(board_alignments, dim=-1, keepdims=True)
        )
        ...
        board_alignments = pad_tensor(board_alignments, axis = 1, min_size = torch.max(decoder_lengths).item())
        decoder_inputs = pad_tensor(decoder_inputs, axis = -1, min_size = 2)
        candidates = pad_tensor(candidates, axis = -1, min_size = self.data_features["max_candidates"])
        ...
        candidates = torch.reshape(candidates, (self.h_params["batch_size"], -1, self.data_features["max_candidates"]))
        candidates = candidates[:, :torch.max(decoder_lengths).item(), :] # torch.int - (b, n_locs, MAX_CANDIDATES)
        
        # Trimming to the maximum number of candidates
        candidate_lengths = torch.sum((torch.gt(candidates, PAD_ID)), -1)    # int32 - (b,)
    ...
    def forward(self, inputs: dict):
        ...
        film_embedding_input = torch.cat([current_power_embedding, current_season_embedding], dim = 1)
                
        # FiLM for board state
        board_film_weights = self.board_encoding_film_weight_layer(film_embedding_input)[:, None, :]
    
        board_film_gammas, board_film_betas = torch.tensor_split(board_film_weights, 2, dim=2)
        ...
        prev_ord_film_weights = self.prev_orders_encoding_film_weight_layer(film_embedding_input)[:, None, :]
        prev_ord_film_weights = torch.tile(prev_ord_film_weights, (self.data_features["n_prev_orders"], 1, 1))
        
        prev_ord_film_gammas, prev_ord_film_betas = torch.tensor_split(prev_ord_film_weights, 2, dim=2)
        prev_ord_film_gammas = torch.split(prev_ord_film_gammas, self.film_output_dims, dim=2)
        prev_ord_film_betas = torch.split(prev_ord_film_betas, self.film_output_dims, dim=2)
        ...
        prev_orders_state = torch.reshape(prev_orders_state, (self.h_params["batch_size"] * self.data_features["n_prev_orders"],
                                                           self.data_features["n_nodes"],
                                                           self.data_features["n_orders_features"]))
        
        prev_ord_conv = self.prev_orders_gcn_conv_layer(
            prev_orders_state, film_gammas = prev_ord_film_gammas, film_betas = prev_ord_film_betas)
        
       
        
        # Splitting into (b, n_prev_orders, N_NODES, attn_size // 2)
        # and reducing over mean of n_prev_orders
        prev_ord_conv = torch.reshape(prev_ord_conv, [self.h_params["batch_size"],
        ...
        return {'board_alignments': inputs['board_alignments'],
                'decoder_inputs': inputs['decoder_inputs'],
