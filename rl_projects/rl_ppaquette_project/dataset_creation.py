...
from utilities.dataset_creation_functions import (
    get_request_id,
    get_policy_data,
    format_datapoint,
    generate_datapoint,
...
dataset_index = {"size_train_dataset": 0, "size_valid_dataset": 0}

# Splitting games into training and validation
phase_count_dataset = pickle.load(
    open(MODEL_DATA_PATHS["PHASES_COUNT_DATASET_PATH"], "rb")
)
...
with concurrent.futures.ProcessPoolExecutor(
    max_workers=DATASET_CREATION_NUM_WORKERS
) as executor:
    while pending_samples != 0:
        ...
        for processed_sample in executor.map(process_sample, local_buffer):
            ...
            for (
                is_validation_datapoint,
                power_name,
                game_result,
            ) in processed_datapoints:
