...
from utilities.data_extraction_functions import (
    convert_to_board_representation,
...
def main():
    ...
    with concurrent.futures.ProcessPoolExecutor(
        max_workers=DATA_EXTRACTION_NUM_WORKERS
    ) as executor:
            ...
            with open(json_file_path, "r") as json_file:
                while True:
                    ...
                    for game_id, saved_game in executor.map(process_game, local_buffer):
                        ...
                        end_supply_centers = get_end_scs_info(
                            saved_game,
                            game_id,
                            all_powers,
                            supply_centers_to_win,
                            end_supply_centers,
                        )
                        ...
                        for phase in saved_game["phases"]:
                            ...
                            hash_table[phase["state"]["zobrist_hash"]] += [
                                "{}/{}".format(game_id, phase["name"])
                            ]
    ...
    pickle.dump(
        end_supply_centers, open(MODEL_DATA_PATHS["END_SCS_DATASET_PATH"], "wb")
    )
